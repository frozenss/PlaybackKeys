...
- **Elevation path:** document that if the Browser gate stays on and External hotkeys fail over an elevated game, run the AHK bridge script as Administrator (or equivalent scheduled-task elevation). Auto `*RunAs` restart inside the generated script is a follow-up, not required to accept this ADR.
