...
8a. As a user who presses main-keyboard OEM punctuation (for example `` ` ``, `.`, `,`, `-`, `=`, `;`, `/`, brackets, quote, backslash) or another physical key with a known AHK mapping, I want that press recorded instead of silently ignored, so that AHK remaps I already use outside Chrome can be configured in the panel (ADR-0005).
8b. As a user whose layout/IME types a glyph such as `·` on the Period key, I want the recording to bind the physical OEM key (Period → `.`), not the produced character, so that the generated hotkey stays valid in AutoHotkey across layouts (ADR-0005).
8c. As a user who presses a key that still cannot be mapped (Dead / IME / `Unidentified`, or a deferred class such as media/lock keys), I want a short “unsupported key” tip while recording stays open—except modifier-only and key-repeat, which stay silent—so that I can tell ignore-from-bug apart from intentional reject.
9. As a user who binds a commonly typed key (for example a bare letter, digit, common typing punctuation, or Shift+letter) as an External hotkey, I want a one-time warning at bind/confirm time that the key will be swallowed globally while a usable browser window exists, so that I understand the trade-off without a permanent banner cluttering the page. Chords that include Ctrl, Alt, or Win do not get this typing-collision warning (shortcut-collision is out of scope for the confirm; AltGr≈Ctrl+Alt is an accepted v1 edge case).
...
- **Recording UX**: Key-capture primary path for External hotkeys and the Bridge toggle hotkey; persist embed-safe AHK syntax plus a human-readable display label (ADR-0005). Capture binds the **physical key** (prefer `KeyboardEvent.code` → AHK key/symbol), not the layout/IME `event.key` character—same spirit as existing numpad handling. Main-keyboard OEM punctuation with a known AHK mapping is capturable; numpad (including NumLock) stays accepted as today. `Intl*` without a fixed AHK name is unsupported (no character fallback). Intentional non-capture: modifier-only, key repeat, Escape (cancels recording), Dead / IME / `Unidentified`, mouse. Media / browser / lock / system keys outside the existing numpad set are deferred (not a silent “punctuation is unsafe” rule—those were an incomplete allowlist). Stored `ahkHotkey` must be safe to embed verbatim in generated `...::` lines (capture escapes `;` so it is not a comment; `` ` `` is stored literally—doubling it is Invalid hotkey in AHK v2); `label` shows ordinary symbols (e.g. `;`, `` ` ``, `.`), not escape markup. On confirming a high-collision External hotkey (letters / digits / common typing punctuation / Space / Enter / Tab / Backspace / Delete, including Shift-only variants), show a one-shot warning at that moment only—not a persistent page banner. Presence of Ctrl, Alt, or Win suppresses the warning (typing-collision only; not a general shortcut denylist). Bridge toggle high-collision uses the same classifier and confirm path with copy that the key is swallowed globally while the AHK bridge script runs (toggle is outside remap `#HotIf`). While recording, unmapped real key presses get a short unsupported tip on the hotkey label and stay in record mode; modifier-only and repeat remain silent.
...
Capture seam (`shared/external-hotkey.js` + `scripts/external-hotkey-test.mjs`) should also cover, when ADR-0005 is implemented: OEM punctuation happy paths; physical-key preference when `event.key` is a layout glyph (e.g. `·` + `code: "Period"` → `.`); embed-safe `;` (`';`) and literal `` ` `` in `ahkHotkey` with symbol `label`; high-collision flag for bare typing punctuation; `Intl*` without a fixed AHK name → `{ unsupported: true }`; intentional silent ignores (modifier-only, repeat) vs unsupported (Dead/IME/unmapped).
...
- ADR-0005 (physical-key capture + embed-safe `ahkHotkey`): implemented in `shared/external-hotkey.js` (capture allowlist + `{ unsupported: true }`) and options recording tip (`ahkUnsupportedKey`).
