...
- **Recording UX**: Key-capture primary path for External hotkeys and the Bridge toggle hotkey; persist AHK syntax plus a display label. On confirming a high-collision External hotkey (letters / common typing keys), show a one-shot warning at that moment only—not a persistent page banner. Bridge toggle high-collision uses the same confirm path with copy that the key is swallowed globally while the AHK bridge script runs (toggle is outside remap `#HotIf`).
